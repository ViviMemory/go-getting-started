...
func (r *AuthPostgres) GetUser(name, phone string) (model.User, error) {
	var user model.User
	query := fmt.Sprintf("SELECT id FROM %s WHERE username=$1 AND phone=$2", userTable)
...
func (r *AuthPostgres) CreateUser(user model.SignUpInput) (int, error) {
	...
	var id int
	query := fmt.Sprintf("INSERT INTO %s (username, phone, role_id, company_id) values ($1, $2, $3, $4) RETURNING id", userTable)

	row := r.db.QueryRow(query, user.Name, user.Phone, user.RoleId, 1)
